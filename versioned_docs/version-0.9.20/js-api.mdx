...
To use these event APIs, you need to enable mouseEvents in your Gosling spec through 
```javascript
{
  tracks:[
    {
      /** track definition */,
      experimental: { mouseEvents: /** enable mouse events **/ }
    }
  ]
}

``` 
`mouseEvents` can ba either a `boolean` variable that enables/disables all mouse events or an `object` that specifies different mouse events:

<TableWrapper GoslingSchema={GoslingSchema} objName='MouseEventsDeep' includeDescription/>
